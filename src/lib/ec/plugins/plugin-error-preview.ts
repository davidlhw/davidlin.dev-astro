...
class SquigglesAnnotation extends ExpressiveCodeAnnotation {
  render({ nodesToTransform }: AnnotationRenderOptions): Parent[] {
    return nodesToTransform.map((node) => {
      return h("span.error-squiggles", node as any) as Parent;
    });
  }
}
...
class ErrorMessageAnnotation extends ExpressiveCodeAnnotation {
  render({ nodesToTransform }: AnnotationRenderOptions): Parent[] {
    return nodesToTransform.map((node) => {
      return h("span.error-message", node as any) as Parent;
    });
  }
}

export function pluginErrorPreview(): ExpressiveCodePlugin {
  return {
    name: "Error Preview",
    baseStyles: `
      .error-squiggles {
